...
imageLogo.Image = "rbxassetid://132731355226015"
...
minimizeButton.MouseButton1Click:Connect(function()
    ...
    if Minimized then
        ...
        minimizeButton.Image = "rbxassetid://103475137342944"
...
sectionIcon.Image = "rbxassetid://132731355226015"
...
dragIcon.Image = "rbxassetid://130773721088241"
...
dropdownIcon.Image = "rbxassetid://132731355226015"
...
toggleInner.Image = "rbxassetid://130773721088241"
...
-- Simpan koneksi agar tidak ganda
local charAddedConn = nil

local function changeAnimations(animName)
...
Section7:Dropdown({
    ...
    List = {
        "Anthro", "Astronaut", "Bubbly", "Cartoony", "Confident", "Cowboy", "Elder", "Ghost",
        "Knight", "Levitation", "Mage", "Ninja", "Patrol", "Pirate", "Popstar", "Princess",
        "Robot", "Sneaky", "Stylish", "Superhero", "Toy", "Vampire", "Werewolf", "Zombie"
    },
    Callback = function(animName)
        if currentAnimName == animName then return end
        currentAnimName = animName

        -- Terapkan langsung ke karakter saat ini
        changeAnimations(animName)
...
-- Pastikan variabel koneksi disiapkan di luar
local followConnection
local playerToFollowConnection

-- Fungsi untuk menghentikan follow
...
function followPlayer(targetPlayer)
    ...
    local function startFollowing()
        ...
        -- Update posisi untuk mengikuti target
    ...
    -- Jalankan sekarang kalau karakter target sudah ada
    if targetPlayer.Character then
        startFollowing()
    end

    -- Dengarkan respawn untuk memulai ulang follow
    if playerToFollowConnection then
        playerToFollowConnection:Disconnect()
    end
    playerToFollowConnection = targetPlayer.CharacterAdded:Connect(function()
        task.wait(1) -- Tunggu karakter benar-benar terbentuk
        startFollowing()
    end)
...
local MAX_FORCE = 100 -- Batas kecepatan gaya abnormal
local MAX_Y_POS = 500
local MIN_Y_POS = -50
...
local function startAntiFling()
    if antiFlingConnection then return end -- Sudah aktif
    ...
    antiFlingConnection = RunService.Heartbeat:Connect(function()
        ...
        if velocity < 40 and humanoid.MoveDirection.Magnitude > 0 then
            lastSafeCFrame = hrp.CFrame
        end

        if velocity > MAX_FORCE or yPos > MAX_Y_POS or yPos < MIN_Y_POS then
...
-- Deteksi saat karakter baru spawn
...
Section:Button({
   Text = "Invisible GUI",
   Callback = function()
   loadstring(game:HttpGet("https://raw.githubusercontent.com/NightCoded/Hellcat/main/tools/Invisible.lua",true))()
   end
})
...
Section:Button({
   Text = "God Mode",
   Callback = function()
   loadstring(game:HttpGet("https://raw.githubusercontent.com/NightCoded/Hellcat/main/tools/GodMode.lua",true))()
   end
})
...
Section:Button({
   Text = "Fling GUI",
   Callback = function()
   loadstring(game:HttpGet("https://github.com/NightCoded/Hellcat/raw/main/tools/Fling.lua"))()
   end
})
